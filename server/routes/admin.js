...
router.get('/leads', authRequired, async (req, res) => {
  ...
  const leads = rows.map((r) => ({
    ...r,
    package: parsePackages(r.package),
...
router.get('/leads/:id', authRequired, async (req, res) => {
  ...
  res.json({ ...row, package: parsePackages(row.package), addons: safeParse(row.addons, []), pricing: safeParse(row.pricing_json, null) });
});
...
// The `package` column stores a JSON array of package keys going forward.
// A handful of early leads (created before multi-select support) have a
// plain string in that column instead — wrap those in an array so the
// admin UI always sees a consistent shape.
